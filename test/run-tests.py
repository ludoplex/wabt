...
class TestInfo(object):
    ...
    def Parse(self, filename):
        ...
        with open(test_path, 'rb') as f:
            ...
            for line in f:
                empty = False
                if m := re.match(b'\\s*\\(;; (STDOUT|STDERR) ;;;$', line.strip()):
                    directive = m[1].decode('utf-8')
                    if directive == 'STDERR':
                        state = 'stderr'
                        continue
                        ...
                        state = 'stdout'
                        continue
                elif m := re.match(b'\\s*;;;(.*)$', line):
                    directive = m[1].decode('utf-8').strip()
                    if state == 'header':
    ...
    for info in infos_to_run:
        ...
        if status.failed > continued_errors:
            if options.fail_fast:
                ...
                if rerun_verbose := YesNoPrompt(
                    question='Rerun with verbose option?', default='no'
                ):
